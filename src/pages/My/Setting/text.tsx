...
function VirtualList() {
  const [dataList, setDataList]: any = React.useState([]); /* 保存数据源 */
  const [position, setPosition] = React.useState([
    0, 0,
  ]); /* 截取缓冲区 + 视图区索引 */
  const scroll = React.useRef(null); /* 获取scroll元素 */
  const box: any = React.useRef(null); /* 获取元素用于容器高度 */
  const context: any =
  ...
  // const handleScroll = () => {
  //   const { scrollTop } = scroll.current ? scroll.current : { scrollTop: 0 };
